...
def sentence1() -> Expr:
    ...
    C = Expr('C')
    a_and_b = disjoin([A,B])
    not_a_iff_nota_or_c = (~A) % disjoin([(~B), C])
    not_a_or_not_b_or_c = disjoin([~A, ~B, C])

    return conjoin([a_and_b, not_a_iff_nota_or_c, not_a_or_not_b_or_c])
...
def sentence2() -> Expr:
    ...
    c_iff_b_or_d = C % disjoin([B, D])
    a_implies_notB_andnotD = A >> conjoin([~B, ~D])
    not_B_and_notC_impliesA = (~(conjoin(B, ~C))) >> A
    notD_impliesC = ~D >> C
    return conjoin(c_iff_b_or_d, a_implies_notB_andnotD, not_B_and_notC_impliesA, notD_impliesC)
    "*** END YOUR CODE HERE ***"
...
def sentence3() -> Expr:
    ...
    PacmanAlive_1 = PropSymbolExpr("PacmanAlive1",0,1) #string, index, time
    PacmanAlive_0 = PropSymbolExpr("PacmanAlive0",1,0)
    PacmanBorn_0 = PropSymbolExpr("Pacmanborn0", 2, 0)
    PacmanKilled_0 = PropSymbolExpr("PacmanKilled0", 3, 0)

    alive1 = Expr('PacmanAlive_1')
    alive0 = Expr('PacmanAlive_0')
    born0 = Expr('PacmanBorn_0')
    killed0 = Expr('PacmanKilled_0')

    first = alive1 % (disjoin(conjoin([alive0, ~killed0]), conjoin([~alive0, born0])))
    second = ~(conjoin([alive0, born0]))
    third = born0
...
def entails(premise: Expr, conclusion: Expr) -> bool:
    ...
    expression = conjoin(premise, ~ conclusion)
    if not findModel(expression):
        return True
    else:
        return False
...
def plTrueInverse(assignments: Dict[Expr, bool], inverse_statement: Expr) -> bool:
    ...
    "*** BEGIN YOUR CODE HERE ***"
    #util.raiseNotDefined()
    #not_inverseStatement = ~ inverse_statement

    not_inverse = ~ inverse_statement
    return pl_true(not_inverse, assignments)
...
def atMostOne(literals: List[Expr]) -> Expr:
    ...
    """
    "*** BEGIN YOUR CODE HERE ***"
    all_combinations = itertools.combinations(literals, 2)
    #need to turn all_combinations into a list

    list1 = list(all_combinations)

    firstLetter = ~list1[0][0]
    secondLetter = ~list1[0][1]
    ...
    for i in range(1, len(list1)):
        ...
        miniExpression = disjoin(firstLetter, secondLetter)
        expression = conjoin(expression, miniExpression)
    return expression
    "*** END YOUR CODE HERE ***"
...
def exactlyOne(literals: List[Expr]) -> Expr:
    ...
    firstStatement = atLeastOne(literals)
    secondStatement = atMostOne(literals)
    return to_cnf(conjoin(firstStatement, secondStatement))
    "*** END YOUR CODE HERE ***"
...
def pacphysicsAxioms(t: int, all_coords: List[Tuple], non_outer_wall_coords: List[Tuple], walls_grid: List[List] = None, sensorModel: Callable = None, successorAxioms: Callable = None) -> Expr:
    ...
    if sensorModel is not None:
        sensorInfo = sensorModel(t, non_outer_wall_coords)
...
def collapseHelper(list, andOr): #and will be true, or will be false
    ...
    if andOr:
        for i in range(1, len(list)):
            answer = conjoin(list[i], answer)
    else:
        for i in range(1, len(list)):
            answer = disjoin(list[i], answer)
...
def foodLogicPlan(problem) -> List:
    ...
    for t in range(0, 50):
        ...
        goal = ~disjoin(allFood)

        goalSentence = conjoin(goal, conjoin(KB))
        model = findModel(goalSentence)
        if model:
            return extractActionSequence(model, actions)

        for (x,y) in non_wall_coords:
            pacmanSucessorStatements.append(pacmanSuccessorAxiomSingle(x,y, t+1, walls))
        KB.append(conjoin(pacmanSucessorStatements))

        # if food was is here, and pacman not there, then there will be food there
        for (x,y) in non_wall_coords:
            PremiseB1 = PropSymbolExpr(food_str, x, y, time = t)
            PremiseB2 = ~PropSymbolExpr(pacman_str, x, y, time = t)
            conclusionB = PropSymbolExpr(food_str, x, y, time = t + 1)
            KB.append(conclusionB % conjoin(PremiseB1, PremiseB2))
